...
if __name__ == '__main__':
    try:
        if not infer:
            model_number = 1
            model_dir_path = util_classification.make_model_dir("./models/DTR", "decision_tree_classification")
        else:
        ...
        if not infer:
            logger.debug(f"Training DTR model")
            ...
            dtr.fit(dataset.train_X, dataset.train_y)

            with open(model_dir_path + "/DTR.pkl", 'wb') as f:
                pickle.dump(dtr, f)
            ...
            logger.debug("Loading DTR model")
            with open(model_dir_path + "/DTR.pkl", 'rb') as f:
                dtr = pickle.Unpickler(f).load()

            main_y_hat = dtr.predict(dataset.test_X)

            dataset.test = util_classification.add_preds(dataset.test, main_y_hat, "DTR")
