...
def evaluate_condition(row, cond):
    ...
    threshold_value = convert_value(threshold_raw)
...
def apply_rules_to_dataset(df, rules_df, target_number):
    ...
    return np.array(preds), matched_rule_ids, matched_supports
...
if __name__ == '__main__':
    try:
        model_dir_path = util_classification.make_model_dir("./models/neuro_symbolic", "fallback_reasoning")
        ...
        elapsed = time.time() - t
        logger.debug('BNN inference elapsed: {}'.format(elapsed))

        output_lines = []

        for target_number in range(1, prediction_window + 1):
            plp_preds, matched_rule_ids, matched_supports = apply_rules_to_dataset(dataset.test,
                                                                                   rules_df,
                                                                                   target_number)
            ...
            final_preds = bnn_preds[:, target_number - 1].copy()
            fallback_mask = final_preds < 0
            final_preds[fallback_mask] = plp_preds[fallback_mask]

            dataset.test[f"BNN_Fallback_target_{target_number}"] = final_preds

            y_true = dataset.test[f"target_class_t+{target_number}"].astype(int).values
            y_pred = dataset.test[f"BNN_Fallback_target_{target_number}"].fillna(-1).astype(int).values
            ...
            logger.debug(f"BNN + Fallback - Prediction Window {target_number}")
        ...
        dataset.test.to_csv(model_dir_path + "/BNN_Fallback_preds.csv", index=False)
