...
if not infer:
    model_number = 1
    model_dir_path = util.make_model_dir("./models/SVR", "SVR")
else:
    model_dir_path = "./models/SVR/model-" + str(model_number)
...
if not infer:
    ...
    for i in range(len(dataset.segment_ids)):
        logger.debug("Training SVR model for segment number {}".format(i+1))
        svr = SVR()
        svr.fit(dataset.train_X, dataset.train_y[:, i])
        svrs.append(svr)
        with open(model_dir_path + "/SVR_segment_{}.pkl".format(i+1), 'wb') as f:
            pickle.dump(svrs[-1], f)
        gc.collect()
    elapsed = time.time() - t
...
# Inference Phase
    ...
    # Load models
    for i in range(len(dataset.segment_ids)):
        with open(model_dir_path + "/SVR_segment_{}.pkl".format(i+1), 'rb') as f:
            unpickler = pickle.Unpickler(f)
            svrs.append(unpickler.load())
    # Predict
    main_y_hat = np.zeros((dataset.test_X.shape[0], len(dataset.segment_ids)))
    for i in range(len(svrs)):
        logger.debug("Predicting SVR model for segment number {}".format(i + 1))
        main_y_hat[:, i] = np.array(svrs[i].predict(dataset.test_X)).flatten()

    # Add predictions to DataFrame
    dataset.test = util.add_preds(dataset.test, main_y_hat, "SVR", dataset.segment_ids)

    # # Round predictions
    # dataset.test = util.round_values(dataset.test, ['SVR'])

    # Calculate Errors
    util.calculate_errors(dataset.test, pred="SVR")

    # Write results out
    util.result_file(dataset.test, model_path=model_dir_path, model_names=["SVR"])

    # # Write predicted data out
    # util.predictions_to_csv(dataset.test, model_dir_path=model_dir_path, file_name="SVR")
