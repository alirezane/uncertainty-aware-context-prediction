...
main_num_neurons = [1500, 1500, 1500]
main_epoch = 500
main_batch_size = 100
main_lr = 0.0001
...
if __name__ == '__main__':
    try:
        ...
        if not infer:
            model_number = 1
            model_dir_path = util.make_model_dir(f"./models/ANN/{model_type.upper()}", "ANN")
        else:
            model_dir_path = f"./models/ANN/{model_type.upper()}/model-" + str(model_number)
        ...
        if not infer:
            ...
            predictor = util.Ann(train_x=dataset.train_X, train_y=dataset.train_y,
        ...
        # Inference Phase
            ...
            # Load predictor
            predictor = util.Ann(model_type=model_type,
                                 train_x=dataset.train_X, train_y=dataset.train_y,
                                 valid_x=dataset.test_X, valid_y=dataset.test_X,
                                 num_neurons=main_num_neurons, epoch=main_epoch,
                                 batch_size=main_batch_size, lr=main_lr, loss=main_loss,
                                 activation=main_activation, patience=main_patience,
            ...
            # main_y_hat = main_y_hat.reshape((main_y_hat.shape[0]*main_y_hat.shape[1], main_y_hat.shape[2]))
            dataset.test = util.add_preds(dataset.test, main_y_hat, "ANN", dataset.segment_ids)
            ...
            # Calculate Errors
            util.calculate_errors(dataset.test, pred="ANN")

            # Write results out
            util.result_file(dataset.test, model_path=model_dir_path, model_names=["ANN"])

            # Write predicted data out
            util.predictions_to_csv(dataset.test, model_dir_path=model_dir_path, file_name="ANN")
